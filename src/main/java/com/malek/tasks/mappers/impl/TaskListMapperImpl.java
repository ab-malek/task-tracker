package com.malek.tasks.mappers.impl;
...
public class TaskListMapperImpl implements TaskListMapper {
    ...
    public TaskListDto toDto(TaskList taskList) {
        ...
        return new TaskListDto(
            ...
            Optional.ofNullable(taskList.getTasks())
                .map(List::size)
                .orElse(0),
            calculateTaksListProgress(taskList.getTasks()),
            Optional.ofNullable(taskList.getTasks())
                .map(tasks -> 
    ...
    private Double calculateTaksListProgress(List<Task> tasks){
        if(null == tasks) return null;
        
        long countClosedTask = tasks.stream().filter(task ->
             TaskStatus.CLOSED == task.getStatus())
             .count();

        return (double) countClosedTask/tasks.size();
    }
